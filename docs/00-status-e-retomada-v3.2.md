...
| ⚠️ **Conferência nos dois temas** | o maestro conferiu o **campo de cargo** em 21/08 e aprovou. O resto do que entrou em 20 e 21/08 segue sem olho humano — sino, painel `/notificacoes`, popover de Filtros, e o **rodapé que passou a rolar** |
| ⚠️ **P-049 — as duas barras de rolagem** | o rodapé dentro do `main` dá ~41px de rolagem a Lista, Kanban e Contatos, que já rolam por dentro. **Previsto, não visto.** Se ficar feio, tirar `rolagem-visivel` do `main` resolve numa linha |
...
✅ **Dados atualizados de novo em 27/08.** A API do Pipedrive **continua viva** — o contrato não fechou. Terceira extração feita; os snapshots de 17/08 e 20/08 ficam preservados em `dados/pipedrive-snapshot-17-08/` e `dados/pipedrive-snapshot-20-08/`. Dos 112 registros que divergiram em sete dias, **os 36 novos entraram** por `scripts/sincroniza-novos.mjs` (idempotente, ensaia por padrão): 2 pessoas, 33 atividades e 1 anotação. A base agora tem **2.461 negócios · 2.897 organizações · 4.606 pessoas · 6.561 atividades · 931 anotações**.

⚠️ **Os alterados e os apagados no Pipedrive continuam fora**, e a conta cresceu: **83 alterados e 1 atividade apagada só entre 20 e 27/08**, somados aos 144 de antes. **Não há como alcançá-los:** o schema não guarda o id do Pipedrive, então não há como dizer que "aquela linha de lá" é "esta daqui" (**P-048**). A divergência cresce a cada dia até a virada.

⚠️ **O alarme do script era um número escrito à mão e virou medição.** Ele conferia se os eventos reais continuavam sendo **9** — valor medido em 20/08 — e por isso gritava toda vez que alguém usava o sistema entre uma rodada e outra; em 27/08 já eram 16, todos de trabalho real. Agora ele **mede antes de começar** e compara com o depois: o que interessa nunca foi quantos eventos existem, e sim se **esta** rodada criou algum — ela não deveria, porque só há `insert` e o gatilho do log é `after update`. Alarme que sempre toca deixa de ser alarme.
...
| ~~P-049~~ | ~~O rodapé dentro do `main` dá ~41px de rolagem às telas de altura cheia~~ | — | ✅ **encerrada 27/08 por D-150, que revoga a D-146.** Deixou de ser hipótese e virou defeito: foi a segunda causa da **C-12**, levando os rótulos do Kanban para fora da tela. O rodapé voltou à coluna externa, largura inteira, e o `main` parou de transbordar — cada tela tem exatamente uma rolagem. Os ~41px voltam a custar em toda tela, e a troca é deliberada |
...
| ~~P-050~~ | ~~297 telefones corrompidos na origem~~ | — | ✅ **encerrada 27/08 por D-154: ficam como estão.** São 168 em notação científica (`56298E+12`) e 129 gravados como `"5"`, de 1.706 telefones. ⚠️ **Não é defeito da carga** — conferido em `persons.json`, o Pipedrive já os tem assim, e não há de onde recuperar. Telefone não é dado relevante nesta operação; apagar destruiria a única pista de que existiu um contato. Aceito junto: clicar num desses abre o WhatsApp com lixo |
