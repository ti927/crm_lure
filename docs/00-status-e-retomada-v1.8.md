...
| **F6 — Atividades** | ✅ **construída 18/08.** Modo lista (agrupado por dia: Vencidas → Hoje → Amanhã → datas) e modo calendário mensal; criação com vínculo opcional a negócio/organização/pessoa (D-108); conclusão; registro retroativo; filtros por situação/tipo/responsável; exportação CSV. **Sem migração** — o schema já sustentava os três vínculos |
...
⚠️ **Interpretação assumida na F6, a confirmar:** ao abrir, a tela mostra as **pendentes de todos os responsáveis**, agrupadas por dia, com o filtro de responsável disponível para estreitar. O maestro pediu "padrão do Pipedrive, por dia" — o agrupamento por dia está fiel; o recorte "de todos" (em vez de "só as minhas") foi escolha minha por serem dois sócios que querem enxergar o que há na mesa. Fácil de inverter se ele preferir "minhas" como padrão. O **calendário** foi feito em visão **mensal** — não a grade semanal com faixas de hora do Pipedrive, que seria esforço além da paridade e ninguém pediu.
...
| 08 | 18/08/2026 | **F6 — Atividades.** Tela própria em dois modos: lista agrupada por dia (Vencidas/Hoje/Amanhã/datas, padrão do Pipedrive) e calendário mensal, alternáveis. Criação em diálogo com tipo, título, data, horas, responsável, descrição e vínculo opcional a negócio/organização/pessoa (D-108) via busca única das três entidades; conclusão otimista pela lista; registro retroativo (data no passado, "já concluída"); filtros por situação, tipo e responsável; exportação CSV (B-085). Sem migração — o schema de 17/08 já tinha `organizacao_id`/`pessoa_id` e o gatilho que encadeia a organização. Verificado por `tsc`, `eslint` e `next build`. | **Verificação visual pendente** (Google OAuth barra o agente); interpretações a confirmar com o maestro (recorte padrão "de todos", calendário mensal); F7 (contatos e produtos) é a próxima; acesso a clientes pelo celular continua sem construir |

---

## Changelog

- **v1.8** — 18/08/2026 — **Sessão 08: F6 construída.** Tela de Atividades em dois modos — lista agrupada por dia (Vencidas → Hoje → Amanhã → datas, no padrão do Pipedrive) e calendário mensal. Criação com campos completos e vínculo opcional a negócio, organização ou pessoa (D-108), conclusão otimista, registro retroativo, filtros por situação/tipo/responsável e exportação CSV (B-085). **Sem migração** — o schema já sustentava os três vínculos desde a carga. ⚠️ Sem verificação visual (Google OAuth barra o agente); interpretações assumidas (recorte "de todos" e calendário mensal) anotadas em 4.2.
