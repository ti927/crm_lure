...
| D-152 | 27/08/2026 | **A ficha da organização passa a cadastrar pessoa e a criar atividade.** A busca de pessoa oferece "Cadastrar *fulano* e vincular" quando não acha ninguém; a seção de Atividades ganha botão de criar e abre em **"minhas pendentes"**, com "todas pendentes" e "todas" à mão | Pedido do maestro. ⚠️ **Nada disso é entidade nova:** `criarPessoa` já existia, e `SeletorAsync` já sabia oferecer "criar o que foi digitado" — é como a ficha do negócio cria organização (D-127). Faltava a ligação. ⚠️ **O botão de cadastrar fica por ÚLTIMO na lista de resultados**, de propósito: nesta base 41% dos cadastros já são duplicata de nome (D-121), então quem existe tem de ser mais fácil de achar do que de recriar. ⚠️ **Atividade pode pertencer a organização sem passar por negócio** (D-108), e não é caso de borda: **55% das atividades da base** estão penduradas em organização. ⚠️ **"Ativa" quer dizer pendente**, e o recorte abre no do usuário logado, coerente com a D-149 — mas comparando com `usuario.id`, **nunca com `auth.uid()`**: desde a D-109 os dois são diferentes, e o errado deixaria a seção vazia justo para quem veio da carga, sem erro nenhum (armadilha da C-05). Se não houver "eu", abre em "todas pendentes" em vez de mostrar vazio | ✅ |
